import pandas as pd
from generate_vocab_list import generate_vocab_list
from fix_word_context import fix_word_context
...
def make_data():
    ...
    filename_context = "corpus_studio_context.csv"

    # import raw csv files
    df_codes_raw = pd.read_csv(f"../data/raw/{filename_codes}")
    df_context_raw = pd.read_csv(f"../data/raw/{filename_context}")
    ...
    # if the first or final words of a file are coded, the pre or post contexts will be NaN, so we need to fill these
    ...
    df_merged.drop(
        columns=["Color", "Weight score", "Modified", "Created", "Area", "Coverage %"],
        inplace=True,
    )
    ...
    # drop redundant columns
    df_merged.drop(
        columns=["Document name", "Code", "Beginning", "Document group"], inplace=True
    )
    ...
    # use vocab list to fix word context issues
    fix_word_context()

    # import df again to make final changes
    df_final = pd.read_csv("../data/interim/corpus_studio_pre_processed.csv")

    df_final["Group"] = df_final["Time"] + df_final["Social group"]
    ...
    df_final.to_csv(
        "../data/processed/corpus_studio_processed.csv", index=False, encoding="UTF-8"
    )
